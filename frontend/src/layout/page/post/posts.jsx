...
const Posts = (props) => {
    ...
    const [autoSearchDebounce] = useDebouncedCallback(autoSearch, 300);
    ...
    return (
        <Grid columns="equal" style={{margin:30}}>
            ...
            <Grid.Column width="8">
                ...
                {
                    loading && [1,2,3,4,5,6,7,8,9,19].map((index) => <Skeleton active={true} />)
                }
                {
                    !loading && posts.length > 0 && posts.length < total ?
                    <Button
                        onClick={()=> setOffset(posts.length)}
                        fluid color={theme} style={{marginTop: 20, height: 150}}
                    >
                        点击查看更多
                    </Button>
                        :
